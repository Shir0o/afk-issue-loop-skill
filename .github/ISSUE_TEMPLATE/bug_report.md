---
name: Bug report
about: The skill did something unexpected or wrong
labels: needs-triage
---

**What did you ask the agent to do?**

<!-- e.g. "run the issue loop" or "process open issues AFK" -->

**What did it actually do?**

<!-- Paste the agent's output or a transcript excerpt -->

**What did you expect instead?**

**Environment**

- Antigravity version (`agy --version`):
- Dependency skills installed: triage / implement / tdd / code-review (check all that apply)
- Repo type (public GitHub / private GitHub / other):

**Additional context**

<!-- Any relevant AGENTS.md, docs/agents/*.md contents, or CI setup -->
